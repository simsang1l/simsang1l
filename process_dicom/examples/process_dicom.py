...
from dicom_key import Base, create_dicom_key_class, Dicomtags
...
def load_dicom_tags_multiprocessing(start_path, tags, start_date: int, end_date: int, modal = ["CR", "MR", "CT"]):
    ...
    # DataFrame으로 변환
    # dicom_tags = pd.DataFrame(valid_results)
...
def save_dicom_tags_to_postgres(input_df, batch_size: int = 100000):
...
def process_dicom(data_tuple):
    ...
    if pd.notna(value["person_id"]) and pd.notna(value["seriesdescription"]):
        try:
            img = pydicom.dcmread(source_filepath, stop_before_pixels=True, specific_tags=["StudyDescription"])  # 태그 정보만 읽음
            seq = source_filepath.split('.dcm')[0]
            # CDM테이블의 key생성
            ...
            ct_patientid_path = os.path.join(file_modal_path, value["patient_no"])
...
def rename_dicom_multiprocessing(save_path, input_df, batch_size = 50000, max_workers=4, modal = ["CT", "MR", "CR"]):
    """
    dicom tag와 CDM을 결합한 데이터프레임을 이용해서 dicom 파일 폴더 및 파일명 수정
    생성되는 파일
    1. 파일명 변경된 이미지
...
if __name__ == '__main__':
    ...
    start_date_str = "20240516" # "20230926" # "20240516"
    end_date_str = "20240531" # "20240515" # "20240531"
    ...
    try :
        ...
        for workdate in date_list:
            ...
            # DICOM 원내 환자번호(patient_no) → CDM patient_id 매핑 쿼리 실행
            # merged_df = search_in_CDM(workdate, workdate)
            # rename_dicom_multiprocessing(args.d, merged_df, modal = ['CT'])
