...
continuous_cols  <- c("gagew", "birth_bmi_zscore", "iarvppd28", "niarvrpd28", "niarvhfnc28") # c("invfpod", "birth_bmi", "stday")
categorical_vars <- c("atbyn", "nrp_grade", "lbp", "Hypotension_Tx", "efy_36", "vent_severe_28", "bdp", "rop_yn") # c("eythtran", "strdu", "resuo", "resui", "atbyn","vent_severe", "vent_severe_36", "lbp")
...
# ---------------- 실행 예 ----------------
# 0 vs 2 (예: Normal vs High)
# ✔️  Bootstrap-corrected C-index: 0.687 

# n=6833   Mean absolute error=0.007   Mean squared error=8e-05
# 0.9 Quantile of absolute error=0.014


# 📊  Calibration metrics (Train bootstrap)
# • Brier / Scaled Brier : 0.1866 / 0.0942
# • Intercept / Slope    : -0.000 / 1.000
# • ECE / MCE            : 0.0134 / 0.0302

custom_labels <- c(
  "gagew" = "Gestational age",
  "birth_bmi_zscore" = "Birth BMI z-score",
  "iarvppd28" = "IMV Duration (DOL 28)",
  "niarvrpd28" = "NIV Duration (DOL 28)" ,
  "niarvhfnc28" = "HFNC Duration (DOL 28)",
  "atbyn" = "Antenatal antibiotic",
  "nrp_grade" = "Resuscitation" ,
  "lbp" = "Hypotension" , 
  "Hypotension_Tx" = "Hypotension Treatment",
  "efy_36" = "Early feeding (PMA 36w)",
  "vent_severe_28" = "Respiratory Support (DOL 28)" , 
  "bdp" = "Bronchopulmonary dysplasia",
  "rop_yn" = "Retinopathy of prematurity"
)
res <- draw_nomogram_vs_zero(class_label = 1, var_labels = custom_labels)  # Train 내부부트스트랩 검증 포함
...
# Test set 준비
# [Test]
# AUROC=0.715, C-index=0.715
# Brier/Scaled=1.1774/-4.7174, ECE/MCE=0.9976/1.0597
...
# 0과 1만 남기기
test <- subset(test, test[[target]] %in% c(0, 1))
...
# label 이진화
test$label <- ifelse(test[[target]] == 1, 1L, 0L)
...
# External set도 동일
# AUROC=0.690, C-index=0.690
# Brier/Scaled=0.9669/-5.9611, ECE/MCE=0.9138/0.9846
...
# 0과 1만 남기기
external <- subset(external, external[[target]] %in% c(0, 1))
...
# label 이진화
external$label <- ifelse(external[[target]] == 1, 1L, 0L)
