...
fit_ols <- ols(
  invfpod ~ rcs(dcd_bmi_zscore, 4) + gagew + sex_sys_val,
  data = train_df,
...
plot(p_rcs,                     # 기본 rms 플롯
     ...
     ylab = "IV feeding period",
     xlim         = c(-6, 6),
     # ylim         = c(0, 1),
     lwd  = 2)

# ---------------------------------------------
x <- train_df$dcd_bmi_zscore + train_df$gagew  # 독립 변수 (BMI z-score)
y <- train_df$invfpod   # 종속 변수 (입원일수)
