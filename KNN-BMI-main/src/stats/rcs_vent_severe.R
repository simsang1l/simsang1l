...
# 더미 변수화
#################
train_df <- dummy_cols(train_df, select_columns = "vent_severe")
colnames(train_df)
...
fit_multinomial <- lrm(
  vent_severe ~ rcs(dcd_bmi_zscore, 3) + gagew + sex_sys_val,
  data = train_df
)
...
train_df_dummy <- dummy_cols(train_df, select_columns = "vent_severe")
dummy_cols <- grep("vent_severe_", names(train_df_dummy), value = TRUE)
